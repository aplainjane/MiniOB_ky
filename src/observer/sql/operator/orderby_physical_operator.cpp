...
{
    std::vector<std::unique_ptr<Expression>> unique_exprs;
    unique_exprs.reserve(aggregate_expressions.size());
    for (Expression *expr : aggregate_expressions) {
        unique_exprs.emplace_back(expr); // 修正这里
    }
    tuple_.init(std::move(unique_exprs));
}
...
RC OrderByPhysicalOperator::fetch_and_sort_tables()
{
  ...
  auto cmp = [&order](const CmpPair &a, const CmpPair &b) {
    ...
    for (size_t i = 0; i < cells_a.size(); ++i) {
      ...
      if (cell_a.compare(cell_b) != 0) {
        return order[i] ? cell_a.compare(cell_b) == -1 : cell_a.compare(cell_b) == 1;
      }
    }
