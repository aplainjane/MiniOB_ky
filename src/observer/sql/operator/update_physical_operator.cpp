...
RC UpdatePhysicalOperator::next()
{
  ...
  while (RC::SUCCESS == (rc = child->next())) {
    ...
    for (int i = 0; i < meta_num; ++i) {
      const FieldMeta &fieldmeta = (*table_field_metas)[i];
      if (0 == strcmp(fieldmeta.name(), target_field_name)) {
        target_index = i;
        break;
      }
    }
    ...
    Record new_record;
    rc = table_->make_record(cell_num, values.data(), new_record);
    if (rc != RC::SUCCESS) {
  ...
  // 先删除旧记录
  for (const Record &record : delete_records) {
  ...
  // 再插入新记录
  for (const Record &record : insert_records) {
